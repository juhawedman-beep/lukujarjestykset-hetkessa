...
const emailSchema = z.object({
  email: z.string().trim().email('Virheellinen sähköposti').max(255),
  password: z.string().min(6, 'Vähintään 6 merkkiä').max(100),
});

const signupSchema = emailSchema.extend({
  displayName: z.string().trim().min(1, 'Nimi vaaditaan').max(100),
});

type LoginForm = z.infer<typeof emailSchema>;
    ...
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-muted/30 px-4 py-8">
      <div className="w-full max-w-md">
        <Link to="/" className="flex items-center justify-center gap-2 mb-6 text-foreground">
          ...
          <h1 className="text-2xl font-bold">Lukujärjestykset hetkessä</h1>
        </Link>
              ...
              <TabsContent value="signup">
                <form onSubmit={signupForm.handleSubmit(handleSignup)} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="signup-name" autoComplete="name" {...signupForm.register('displayName')} />
